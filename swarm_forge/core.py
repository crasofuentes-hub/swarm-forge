...
class SwarmEngine:
    ...
    def run_cycle(self, cycle_index: int) -> Dict[str, Any]:
        ...
        if getattr(self, "disable_patch_apply", False):
            applied_records = []
            for d in decisions:
                if d.applied:
                    d.applied = False
                    d.reason = "patch_apply_disabled_from_cli"
        else:
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--guarded-tol-abs", type=float, default=0.002)    parser.add_argument("--guarded-tol-drift", type=float, default=0.010)
    parser.add_argument("--disable-patch-apply", action="store_true", default=False)
    return parser
...
def main() -> None:
    ...
    engine.guarded_tol_abs = float(args.guarded_tol_abs)    engine.guarded_tol_drift = float(args.guarded_tol_drift)
    engine.disable_patch_apply = bool(args.disable_patch_apply)    if args.resume:
